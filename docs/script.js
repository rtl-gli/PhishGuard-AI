...
async function analyseURL() {
    ...
    if (!url) {
        errorMessage.textContent =
            "Paste a link first so we can check it.";
...
function displayResults(data) {
    ...
    document.querySelector(".risk-icon").textContent =
        data.prediction === 1
            ? "!"
            : "✓";
...
function renderExplanations(items) {
    ...
    if (!items || !items.length) {
        explanations.textContent =
            "No additional feature information was returned.";
    ...
    items.forEach((item, index) => {
        ...
        explanation.textContent =
            `Value: ${Number(item.value).toFixed(2)} · Model importance: ${(item.importance * 100).toFixed(1)}%`;
...
function formatFeatureValue(key, value, url) {
    ...
    if (key === "has_at_symbol") {
        return value
            ? "Present"
            : "Not present";
    }

    if (key === "has_double_slash_redirect") {
        return value
            ? "Possible redirect"
            : "Not detected";
    }
...
function renderHistory() {
    ...
    history.forEach((item) => {
        ...
        risk.className =
            `history-risk ${item.risk_level.toLowerCase()}`;
